...
{
    public class BuyBackContactPaymentHamburgerMenuItem : HamburgerMenuItemBase, IRegionManagerAware
    {
        public BuyBackContactPaymentHamburgerMenuItem()
        {
            Command = new DelegateCommand(Navigate);
            Label = "Platnosc i dokumenty";
            Icon = new PackIconMaterial { Kind = PackIconMaterialKind.CreditCard };
        }

        public override string DefaultNavigationPath => nameof(BuyBackContractPayment);
